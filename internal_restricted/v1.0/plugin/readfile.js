...
f46_0:"Welcome to lesson seven on accuracy assessment and object validity. It is the concluding lesson of our introductory OBIA course. After defining some key concepts we are looking at several options for quantifying accuracy. In order to quantify we need a reliable reference map, sometimes referred to as ground truth. This could be another mapping product or self-taken measurements in the field. Even we assume our reference data set is correct, eventually there are inherent errors present as well. It is always recommended to familiarise with the quality of the reference data upfront. As an alternative to external data sources, many applications use visual interpretation of the image as a reference. Ideally, to minimize biases, image classification and visual interpretation are two separate processes to be carried out independently from each other. In terms of accuracy measures, we differentiate between spatial implicit, or global measures, and spatial explicit ones. Spatial explicit measures are also known as site-specfic measures. In picksel-based analysis, site-specific measures are checking pixel by pixel for correctness, and then an aggregated index is calculated. Non-site specific measures would compare the area covered by each of the classes and then compare the percentages. Since this does not take into account the actual spatial distribution of the classes to be evaluated, it is considered a spatial explicit method. Generally, we would prefer spatial explicit methods over implicit ones to have a clearer understanding of how the overall quality assessment is composed and influenced. An additional challenge in object-based image analysis is to evaluate objects instead of single pixels. Objects can be correctly or incorrectly classified, just like pixels. However, the delineation process itself, is another aspect to consider. Objects can be correctly labelled, for example as a forest or a grassland patch, but are they also delineated in a proper way? We shall see later on that it is not trivial to determine the correctness of the delineation. In fact, due the phenomenon of generalisation in spatial representations, there is no absolutely correct way of generating an object boundary. Rather it is a matter of appropriateness or fitness considering the scale of investigation. Whilst in pixel-based analysis we have to deal with semantic agreements between class labels and a potential mixed-pixel effect, in OBIA we are also facing geometric problems.",
	//7.1 Definitions
	f47_0:"Please read through the definitions yourself.",
