...
f39_0:"Welcome to lesson 6, object-based classification, the second central pillar of the OBIA process. This is the content of lesson 6, organized in six subchapters: (1) Strengths of object-based classification. (2) Sample- vs. rule-based classification. (3) Fuzzy vs. crisp classification. (4) Class hierarchy. (5) Class-related features. (6) Class modelling. As any classification process in remote sensing, also object-based classification aims at delivering information classes. We will realize, however, that we can use much more features for classification as we can use in a pixel-based processing environment. We have provided an overview on object features in the last lesson on image segmentation. All these features can be used for classifying objects, but of course, not all of them make sense. If, for example, we want to distinguish a soccer field from a grassland patch, we can use the rectangular shape and the typical size as a distinctive feature. On the other hand, when considering the fact that a soccer field is surrounded by built-up structures, we may fail in cases where the soccer field is situated in a larger recreational area. So, we need to be aware which features are distinctive, and which are more ad hoc. Of course, we could use such individual features for even more specifying such objects, for example telling soccer stadiums apart from recreational soccer fields. As we will see, object-based classification goes beyond a simple labelling of generated objects. The reason being that often the final classes of concern are more complex, the object delineations are limited, or we do not receive a proper delineations at all, as in the case of conceptual boundaries.  ",
	...
	f43_0:"First, we would like to have a closer look at the concept of class hierarchy. ",
		...
		// 6.4.1 Class hierarchy
			f43_1:"In general a class hierarchy resembles the relationships of classes in a hierarchical system. This implies that classes are not only nominal categories, existing next to each other. They are instead organised in a hierarchical structure, reflecting either their relationship in terms of class definitions or semantics. Here we use the term ‘hierarchy’ in a more taxonomic meaning, to describe a hierarchical classification scheme in the sense of an ontological inheritance system. When looking at remote sensing data that represent real world geographical objects, we may distinguish between two aspects of a class hierarchy. ",
...
f46_0:"Welcome to lesson seven on accuracy assessment and object validity. It is the concluding lesson of our introductory OBIA course. After defining some key concepts we are looking at several options for quantifying accuracy. In order to carry out accuracy assessment we need a reliable reference map, sometimes referred to as ground truth. This could be an existing mapping product or self-taken measurements in the field. Even we assume our reference data set to be correct, eventually there are inherent errors present as well. It is always recommended to check the quality of the reference data beforehand. As an alternative to external data sources, many applications use visual interpretation of the image as a reference. Ideally, to minimize biases, image classification and visual interpretation are two separate processes and carried out independently. In terms of accuracy measures, we differentiate between spatial implicit, or global measures, and spatial explicit measures. Spatial explicit measures are also known as site-specfic measures. In picksel-based analysis, individual picksels are checked for correctness, and then an aggregated index is calculated. Non-site specific measures would compare the area covered by each of the classes and then compare the percentages. This does not take into account the actual spatial distribution of the classes to be evaluated; thus it is a spatial implicit method. Generally, we would prefer spatial explicit methods over implicit ones, to have a clear understanding of how the overall quality assessment is composed and influenced. An additional challenge in object-based image analysis is to evaluate objects instead of single picksels. Objects can be correctly or incorrectly classified, just like picksels. However, the delineation process itself, is another aspect to consider. Objects can be correctly labelled, for example as a forest or a grassland patch, but are they also delineated in a proper way? We shall see later that it is not trivial to determine the correctness of the delineation. In fact, due to the phenomenon of generalisation in spatial representations, there is no absolute correct way of generating an object boundary. Rather it is a matter of appropriateness or fitness, considering the scale of investigation. Whilst in picksel-based analysis we have to deal with semantic agreements between class labels and a potential mixed-picksel effect, in OBIA we also have to tackle with geometric problems.",
	...
	f50_0:"We are now discussing how we construct and read an error matrix.",
		...
		f50_1:"The error matrix is a product of aggregated locally determined errors. The matrix structure allows for emphasizing different aspects in interpreting the outcomes of the assessment. Depending on which viewpoint we apply, we derive different accuracy figures. For example, when looking from the perspective of a producer, we are interested in the quality of our product, as this is the one we can influence. From the perspective of a customer, or user, they would be interested in the quality level, which they can expect with respect to the so-called ground truth.",

		//7.4.2 Error matrix part 2
		f50_2:"The measures presented above are not sensitive to the fact that a certain portion of the agreement occurs by chance. In other words, every random classifier would generate some matching pairs between the classified and the reference image. If we have a binary classification, like vegetation versus non-vegetation, chances are 50 percent that with a random classifier we hit the right class. This probability drops with increasing number of classes, but with 5 classes we still have a 20 percent probability to classify correctly - even the classifier works absolutely random. In order to compensate for this effect, the kappa index was proposed. Kappa is a normalised form of an accuracy measure that takes into consideration chance agreement. ",

		//7.4.3 Error matrix part 3
		f50_3:"Even though the error matrix is widely used to assess the result of a digital image classification, it has some limitations. The error matrix is not a standardised measure that would balance out all kinds of influencing factors, such as sampling size, evenness in class distribution, semantic uncertainty, and others. Further problems arise when the reference data are not perfect, which is usually the case. Here we touch upon the paradox of remote sensing. If we had a perfect reference map of a certain area, there is no need for an update using remote sensing. That means, in reality, reference data are either older, coarser, or less accurate than our product. However, with bad reference data we cannot properly assess our product, which limits the trustfulness and usability. Provokingly we may say that, a poorly evaluated remote sensing product is useless, while a perfectly evaluated one is needless.",

	// 7.5 Towards object-based accuracy assessment
	f51_0:"In an object-based environment, the classical site-specific accuracy assessment would fall short. In other words, checking the classification label at various random locations is not object-specific. Why not? Locations are zero-dimensional points in space, where any two-dimensional aspect is ignored. Meaning that checking individual locations is agnostic to object delineation. When moving towards object-based accuracy assessment, we need to find ways how an object's geometry and spatial extent can be considered. ",

		//7.5.1 Towards object-based accuracy assessment- part 1
		f51_1:"Object-based accuracy assessment has two main components, first the thematic or semantic label, and second the spatial properties of the classified objects. The label aspect is similar to site-specific pixel-based methods, so here we focus on the geometric aspect. There are several ways to compare the spatial characteristics of two corresponding objects. But also several challenges, as we shall see a little later.",
